...
public class Client_DAO {
    ...
    private void create_client_table()
    {
        try
        {
            conn = cConnection.getConnection();
            create_stmt = conn.createStatement();
            String  create_client_table_sql ="CREATE TABLE IF NOT EXIST Client(Name varchar(100),LastName varchar(100),Address varchar(100),Acc_num int primary key)";
    ...
    private boolean add_Client(Client obj_client)
    {
        try
        {
            conn = cConnection.getConnection();
            String add_client_data ="INSERT INTO Client(Name,LastName,Address,Acc_num) VALUES(?,?,?,?)";
